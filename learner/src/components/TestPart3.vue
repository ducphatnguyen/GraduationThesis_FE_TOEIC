                    ...
                    <div class="col-sm-12" v-for="(groupQuestions, groupId) in groupedQuestions" :key="groupId">
                        ...
                        <div class="audio-container mt-5 mb-2">
                            <audio controls style="width:400px">
                                <source :src="getAudioUrl(groupQuestions[0].questionGroup.groupAudio)" type="audio/mpeg" />
                                Your browser does not support the audio element.
                            </audio>
                        </div>

                        <div class="row" v-if="groupQuestions[0].questionGroup.groupImage">
                            <div class="col-md-6 bg-light rounded">
                                <div class="audio-image-container mt-5">
                                    <div class="image-container" v-if="shouldDisplayGroupContent(groupQuestions)">
                                        <img :src="getImageUrl(groupQuestions[0].questionGroup.groupImage)"
                                            style="width: 100%" alt="Luyện thi Listening TOEIC" class="question-image"
                                            loading="lazy" />
                                    </div>
                                </div>
                            </div>
                        ...
                        <button v-if="isGroupAnswered(groupQuestions)" @click="toggleGroupScript(groupId)"
                            class="btn btn-link btn-sm link-offset-3">
                            {{ showGroupScript[groupId] ? 'Hide GroupScript' : 'Show GroupScript' }}
                        </button>
                        <div v-if="isGroupAnswered(groupQuestions) && showGroupScript[groupId]" class="transcript mb-2">
                            <div class="transcript-original">
                                <div v-html="groupQuestions[0].questionGroup.groupScript"></div>
                            </div>
                            <div class="transcript-translation"
                                v-if="groupQuestions[0].questionGroup.translatedGroupScript">
                                <span class="badge text-bg-success">Bản dịch</span>
                                <div v-html="groupQuestions[0].questionGroup.translatedGroupScript"></div>
                            </div>
                        </div>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const toggleGroupScript = async (groupId) => {
            ...
            if (showGroupScript.value[groupId]) {
                console.log(groupedQuestions.value[groupId])
                const groupQuestions = groupedQuestions.value[groupId];
                const groupScript = groupQuestions[0].questionGroup.groupScript; 
                const targetLanguage = "vi"; // Tiếng Việt
                try {
                    const translatedGroupScript = await props.translateText(groupScript, targetLanguage);
