            ...
            <div class="card border-0">
                <div class="row">
                    ...
                    <div class="col-4 mt-4 d-flex justify-content-end">
                        ...
                        <div class="modal zoom" id="addTestModal" tabindex="-1" aria-labelledby="addTestModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="addTestModalLabel"><i
                                                class="fa-solid fa-circle-plus text-success"></i> Add Test
                                        </h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <TestAdd :sectionId="sectionId" :retrieveTests="retrieveTests" :tests="tests" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <table class="table text-center table-hover shadow">
                        ...
                        <tbody>
                            <tr v-for="(test, index) in paginatedTests" :key="test.testId"
                                class="table-row shadow-on-hover align-middle">
                                <td>{{ (currentPage - 1) * ITEMS_PER_PAGE + index + 1 }}</td>
                                <td>{{ test.testName }}</td>
                                <!-- <td><i class="fa-solid fa-circle-user fa-2x"></i><br>{{ test.testParticipants + 100 }}</td> -->
                                <td>
                                    <span v-if="test.testStatus === 1" @click="toggleStatus(test.testId, 0)"
                                        class="btn badge text-bg-success">
                                        Enable
                                    </span>
                                    <span v-else-if="test.testStatus === 0" @click="toggleStatus(test.testId, 1)"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <!-- Mở modal chỉnh sửa Test -->
                                        <button type="button" class="btn btn-white border-0" data-bs-toggle="modal"
                                            :title="`Chỉnh sửa [${test.testName}]`"
                                            :data-bs-target="'#editTestModal-' + test.testId">
                                            <i class="fas fa-edit" style="color: rgb(192, 129, 13)"></i>
                                        </button>
                                        <!-- Modal chỉnh sửa Test -->
                                        <div :id="'editTestModal-' + test.testId" class="modal zoom" tabindex="-1"
                                            :aria-labelledby="'editTestModalLabel-' + test.testId" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5"
                                                            :id="'editTestModalLabel-' + test.testId"><i class="fas fa-edit"
                                                                style="color: rgb(192, 129, 13)"></i> Edit Test</h1>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                    </div>
                                                    <TestEdit :testId="test.testId" :sectionId="sectionId"
                                                        :retrieveTests="retrieveTests" :tests="tests" />
                                                </div>
                                            </div>
                                        </div>
                                        <button type="button" @click="deleteTest(test.testId)"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <router-link :to="{
                                            ...
                                            params: { sectionId: test.section.id, testId: test.testId }
                                        }">
                                            <button class="glowing-button ms-2">Indicate Questions</button>
                                        </router-link>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <!-- Pagination controls -->
                    <nav aria-label="Page navigation" v-if="filteredTests.length > 0">
                    ...
                    <div class="d-flex justify-content-center mt-3 fw-lighter fst-italic" v-if="filteredTests.length > 0">
                        <p>
                            {{ (currentPage - 1) * ITEMS_PER_PAGE + 1 }} -
                            {{
                                Math.min((currentPage - 1) * ITEMS_PER_PAGE + ITEMS_PER_PAGE, filteredTests.length)
                            }}
                            trên {{ filteredTests.length }} kết quả
                        </p>
                    </div>
...
import TestAdd from "@/views/testbysection/TestAdd.vue";
import TestEdit from "@/views/testbysection/TestEdit.vue";
...
export default defineComponent({
    ...
    setup(props, context) {
        const tests = reactive(props.tests);
        console.log(tests);
        const searchText = ref("");
        const filteredTests = computed(() => {
            if (!searchText.value) {
                return reactive(props.tests);
            }
        ...
        const totalPageCount = computed(() => Math.ceil(filteredTests.value.length / ITEMS_PER_PAGE.value));
        ...
        const toggleStatus = async (testId, newStatus) => {
            try {
                ...
                props.retrieveTests();
                // const testToUpdate = props.tests.find((test) => test.testId === testId);
                // if (testToUpdate) {
                //     // testToUpdate.testStatus = newStatus;

                //     //Chưa update được ngày chỉnh sửa
                // }
            } catch (error) {
                console.error(error);
            }
