            ...
            <div class="card border-0">
                <div class="row">
                    ...
                    <div class="col-3 mt-4 d-flex justify-content-end">
                        ...
                        <div class="modal zoom" id="addSectionModal" tabindex="-1" aria-labelledby="addSectionModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="addSectionModalLabel"><i class="fa-solid fa-circle-plus text-success"></i> Add Section
                                        </h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <SectionAdd :retrieveSections="retrieveSections" :sections="sections" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <table class="table text-center table-hover shadow">
                        ...
                        <tbody>
                            <tr v-for="(section, index) in paginatedSections" :key="section.id" 
                                class="table-row shadow-on-hover align-middle">
                                <td>{{ (currentPage - 1) * ITEMS_PER_PAGE + index + 1 }}</td>
                                <td>{{ section.name }}</td>
                                <td>
                                    <img :src="getImageUrl(section.image)" alt="Section Image" class="section-image rounded-5" />
                                </td>
                                <td>
                                ...
                                <td>
                                    <span v-if="section.status === 1" @click="toggleStatus(section.id, 0)"
                                        class="btn badge text-bg-success">
                                        Enable
                                    </span>
                                    <span v-else-if="section.status === 0" @click="toggleStatus(section.id, 1)"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        ...
                                        <button type="button" class="btn btn-white border-0" data-bs-toggle="modal" :title="`Chỉnh sửa [${section.name}]`"
                                            :data-bs-target="'#editSectionModal-' + section.id">
                                            <i class="fas fa-edit" style="color: rgb(192, 129, 13)"></i>
                                        </button>
                                        <!-- Modal chỉnh sửa Section -->
                                        <div :id="'editSectionModal-' + section.id" class="modal zoom" tabindex="-1"
                                            :aria-labelledby="'editSectionModalLabel-' + section.id" aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5 "
                                                            :id="'editSectionModalLabel-' + section.id"><i class="fas fa-edit" style="color: rgb(192, 129, 13)"></i> Edit Section</h1>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" 
                                                            aria-label="Close"></button>
                                                    </div>
                                                    <SectionEdit :sectionId="section.id"
                                                        :retrieveSections="retrieveSections"/>
                                                </div>
                                            </div>
                                        </div>

                                        <button type="button" @click="deleteSection(section.id)" class="btn btn-white border-0"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <router-link :to="{ name: 'admin.lesson.all', params: { sectionId: section.id } }">
                                            <button class="glowing-button">Lesson</button>
                                        </router-link>

                                        <router-link
                                            :to="{ name: 'admin.question.all', params: { sectionId: section.id } }">
                                            <button class="glowing-button ms-2">Question</button>
                                        </router-link>
                                        <router-link :to="{ name: 'admin.test.all', params: { sectionId: section.id } }">
                                            <button class="glowing-button ms-2">Test</button>
                                        </router-link>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <!-- Pagination controls -->
                    <nav aria-label="Page navigation" v-if="filteredSections.length > 0">
                    ...
                    <div class="d-flex justify-content-center mt-3 fw-lighter fst-italic"
                        ...
                        <p>
                            ...
                            {{
                                Math.min((currentPage - 1) * ITEMS_PER_PAGE + ITEMS_PER_PAGE, filteredSections.length)
                            }}
                            trên {{ filteredSections.length }} kết quả
                        </p>
                    </div>
...
import SectionAdd from "@/views/section/SectionAdd.vue";
import SectionEdit from "@/views/section/SectionEdit.vue";
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const totalPageCount = computed(() => Math.ceil(filteredSections.value.length / ITEMS_PER_PAGE.value));
        ...
        const paginatedSections = computed(() => {
            ...
            return filteredSections.value.slice(startIndex, endIndex);
        });
        ...
        const lastRowNumber = computed(() =>
            Math.min((currentPage.value - 1) * ITEMS_PER_PAGE.value + ITEMS_PER_PAGE.value, filteredSections.length)
        );
        ...
        const toggleStatus = async (sectionId, newStatus) => {
            try {
                ...
                const sectionToUpdate = props.sections.find((section) => section.id === sectionId);
                if (sectionToUpdate) {
                    sectionToUpdate.status = newStatus;
                }
            } catch (error) {
                console.error(error);
            }
...
@keyframes glowing {
    ...
    50% {
        box-shadow: 0 0 50px #2f94d8, 0 0 40px #3098dd;
    }
