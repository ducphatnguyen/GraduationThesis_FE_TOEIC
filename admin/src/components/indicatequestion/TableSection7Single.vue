            ...
            <template v-for="(groupedQuestions, groupId, groupIndex) in groupedQuestionMap" :key="groupId">
                <tr v-for="(question, index) in groupedQuestions" :key="question.questionId"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <button style="font-size: 12px" class="btn btn-success rounded-5 disabled">{{
                            question.usage }}</button>
                    </td>
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                            :value="question.questionId"
                            :disabled="selectedQuestions.length >= 29 && !selectedQuestions.includes(question.questionId)"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <div :title="question.questionGroup.groupPassage"
                            v-html="getLimitedPassage(question.questionGroup.groupPassage)"></div>
                    </td>
                </tr>
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 29; // Số lượng câu hỏi cần chọn

            const allGroups = Object.keys(groupedQuestionMap.value);
            const groupCounts = {};
            ...
            const eligibleGroups = allGroups.filter(groupId => groupCounts[groupId] >= maxSelected);
            if (eligibleGroups.length < 4) {
                alert("Không có đủ nhóm câu hỏi có ít nhất " + maxSelected + " câu hỏi để random.");
                return;
            }

            // Chọn ngẫu nhiên 4 nhóm từ danh sách các nhóm đủ điều kiện
            const randomGroups = [];
            while (randomGroups.length < 4) {
            ...
            for (const groupId of randomGroups) {
                ...
                while (randomIndexes.length < Math.ceil(maxSelected / 4)) {
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 29) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
                    text: 'Vui lòng chọn đủ 29 câu hỏi trước khi submit.',
                });
                return;
            }
            try {
                ...
                toast.success('Chỉ định câu hỏi Phần 7 đoạn đơn thành công ', {
                    autoClose: 1000
                });
            } catch (error) {
                console.log(error);
                toast.error('Chỉ định câu hỏi Phần 7 đoạn đơn thất bại ', {
                    autoClose: 1000
                });
            }
