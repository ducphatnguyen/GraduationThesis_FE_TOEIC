            ...
            <tr v-for="(question, index) in paginatedQuestions" :key="question.questionId" 
                ...
                <td>
                    <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                        ...
                        :disabled="selectedQuestions.length >= 5 && !selectedQuestions.includes(question.questionId)">
                </td>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 5; // Số lượng checkbox cần chọn
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 5) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
