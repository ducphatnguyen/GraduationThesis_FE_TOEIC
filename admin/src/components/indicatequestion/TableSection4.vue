                ...
                <tr v-for="(question, index) in groupedQuestions" :key="question.Id"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                            ...
                            :disabled="selectedQuestions.length >= 30 && !selectedQuestions.includes(question.questionId)"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <div :title="question.questionGroup.groupScript"
                            v-html="getLimitedScript(question.questionGroup.groupScript)"></div>
                    </td>
                </tr>
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 30; // Số lượng câu hỏi cần chọn
            ...
            for (const groupId of randomGroups) {
                ...
                while (randomIndexes.length < Math.ceil(maxSelected / 10)) {
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 30) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
                    text: 'Vui lòng chọn đủ 30 câu hỏi trước khi submit.',
                });
                return;
            }
            try {
                ...
                toast.success('Chỉ định câu hỏi Phần 4 thành công ', {
                    autoClose: 1000
                });
            } catch (error) {
                console.log(error);
                toast.error('Chỉ định câu hỏi Phần 4 thất bại ', {
                    autoClose: 1000
                });
            }
