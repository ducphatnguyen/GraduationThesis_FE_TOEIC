    ...
    <table class="table text-center table-bordered shadow">
        <thead class="shadow">
            <tr class="align-middle">
                <th>
                    <button class="btn btn-primary rounded-5 disabled">USED</button>
                </th>
                <th>SELECTED {{ selectedQuestions.length }}/10</th>
                <th>No.</th>
                <th>CONTENT</th>
                <th>A</th>
                <th>B</th>
                ...
                <tr v-for="(question, index) in groupedQuestions" :key="question.questionId" 
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                            ...
                            :disabled="selectedQuestions.length >= 10 && !selectedQuestions.includes(question.questionId)"
                    ...
                    <td class="question-image" v-if="index === 0" :rowspan="groupedQuestions.length">
                        <!-- Display the groupImage only once for each groupId -->
                        <img :src="getImageUrl(question.questionGroup.groupImage) || ' '" style="width:100px" />
                    </td>
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <!-- Display the groupScript only once for each groupId -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const fetchQuestions = async () => {
            ...
            // Extract questionIds and set them as selectedQuestions
            selectedQuestions.value = questions.map(question => question.questionId);
            await fetchQuestionUsageCounts(); // Gọi hàm để lấy số lần sử dụng của các câu hỏi
        };

        const isSubmitEnabled = ref(false);
        const autoRandom = () => {
            const maxSelected = 10; // Số lượng câu hỏi cần chọn
            ...
            for (const groupId of randomGroups) {
                ...
                while (randomIndexes.length < Math.ceil(maxSelected / 2)) {
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 10) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
                    text: 'Vui lòng chọn đủ 10 câu hỏi trước khi submit.',
                });
                return;
            }
            try {
                ...
                toast.success('Chỉ định câu hỏi Phần 7 đoạn kép thành công ', {
                    autoClose: 1000
                });
            } catch (error) {
                console.log(error);
                toast.error('Chỉ định câu hỏi Phần 7 đoạn kép thất bại ', {
                    autoClose: 1000
                });
            }
