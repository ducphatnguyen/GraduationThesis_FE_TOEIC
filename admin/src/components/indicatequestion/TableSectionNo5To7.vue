        ...
        <tbody>
            <template v-for="(groupedQuestions, groupId, groupIndex) in groupedQuestionMap" :key="groupId">
                <tr v-for="(question, index) in groupedQuestions" :key="question.questionId"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length"> <button style="font-size: 12px"
                            class="btn btn-success rounded-5 disabled">{{
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                            ...
                            :disabled="selectedQuestions.length >= 3 && !selectedQuestions.includes(question.questionId)"
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <div :title="question.questionGroup.groupText"
                            v-html="getLimitedPassage(question.questionGroup.groupText)"></div>
                    </td>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 3; // Số lượng câu hỏi cần chọn
