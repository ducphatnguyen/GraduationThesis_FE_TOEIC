    ...
    <table class="table text-center table-hover shadow">
        ...
        <tbody>
            <tr v-for="(question, index) in paginatedQuestions" :key="question.questionId"
                ...
                <td><button style="font-size: 12px" class="btn btn-success rounded-5 disabled">{{ question.usage }}</button>
                </td>
                <td>
                    <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                        :value="question.questionId"
                        :disabled="selectedQuestions.length >= 6 && !selectedQuestions.includes(question.questionId)">
                </td>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 6; // Số lượng checkbox cần chọn
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 6) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
            ...
            try {
                ...
                toast.success('Chỉ định câu hỏi Phần 1 thành công ', {
                    autoClose: 1000
                });
            } catch (error) {
                console.log(error);
                toast.error('Chỉ định câu hỏi Phần 1 thất bại ', {
                    autoClose: 1000
                });
            }
