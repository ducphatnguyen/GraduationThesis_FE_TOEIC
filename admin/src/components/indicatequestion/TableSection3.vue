                ...
                <tr v-for="(question, index) in groupedQuestions" :key="question.questionId" 
                    ...
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <input type="checkbox" :id="'checkbox_' + question.questionId" v-model="selectedQuestions"
                            ...
                            :disabled="selectedQuestions.length >= 39 && !selectedQuestions.includes(question.questionId)"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const autoRandom = () => {
            const maxSelected = 39; // Số lượng câu hỏi cần chọn
            ...
            for (const groupId of randomGroups) {
                ...
                while (randomIndexes.length < Math.ceil(maxSelected / 13)) {
        ...
        const submitQuestions = async () => {
            if (selectedQuestions.value.length < 39) {
                Swal.fire({
                    icon: 'error',
                    title: 'Lỗi',
                    text: 'Vui lòng chọn đủ 39 câu hỏi trước khi submit.',
                });
                return;
            }
            try {
                ...
                toast.success('Chỉ định câu hỏi Phần 3 thành công ', {
                    autoClose: 1000
                });
            } catch (error) {
                console.log(error);
                toast.error('Chỉ định câu hỏi Phần 3 thất bại ', {
                    autoClose: 1000
                });
            }
