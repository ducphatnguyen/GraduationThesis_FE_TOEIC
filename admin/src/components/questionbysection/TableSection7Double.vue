        ...
        <tbody>
            <template v-for="(groupedQuestions, groupId, groupIndex) in groupedQuestionMap" :key="groupId">
                <tr v-for="(question, index) in groupedQuestions" :key="question.Id" 
                    ...
                    <!-- <td v-if="index === 0" :rowspan="groupedQuestions.length"> -->
                    <td v-if="index === 0" :rowspan="groupedQuestions.length">
                        <!-- Buttons in the first row of each group -->
                                        ...
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5"
                                                :id="'editQuestionModalLabel-' + question.questionGroup.groupId"><i class="fas fa-edit"
                                                    style="color: rgb(192, 129, 13)"></i> Edit Question Section 7 Double</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <QuestionEditSection7Double :groupId="question.questionGroup.groupId" :sectionId="sectionId"
...
import QuestionEditSection7Double from "@/views/questionbysection/QuestionEditSection7Double.vue";
...
export default defineComponent({
    ...
    setup(props) {
        const getLimitedPassage = (groupPassage) => {
            ...
            if (groupPassage.length > MAX_SCRIPT_LENGTH) {
                return groupPassage.slice(0, MAX_SCRIPT_LENGTH) + '...';
            } else {
                return groupPassage;
            }
        };
